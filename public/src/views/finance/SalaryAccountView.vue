...
<template>
    <div style="margin-top: -30px">
            ...
            <el-table-column fixed="right" label="操作" width="135">
                <template v-slot="scope">
                    ...
                    <el-popconfirm confirm-button-text="确定" cancel-button-text="取消" icon-color="red" title="确定删除该条信息吗"
                        @confirm="deleteClick(scope.row)" @cancel="cancleDelete()">
                        <template #reference>
        ...
        <el-dialog title="添加工资账套" width="50%" v-model="dialogVisible"
            style="display: flex; justify-content: space-around; align-items: center">
            <div style="height: 300px">
                <el-steps direction="vertical" :active="activeindex">
                    <el-step :title="item" v-for="(item,index) in salaryItem" :key="index" />
                </el-steps>
                <el-input :placeholder=" '请输入' + salaryItem[index]" v-for="(val, key, index) in salary" :key="index"
                    v-show="activeindex == index" style="float: right; width: 400px; margin-top: -150px"
                    v-model="salary[key]"></el-input>
            </div>
        ...
        <el-dialog title="编辑工资账套" width="50%" v-model="editdialogVisible"
            style="display: flex; justify-content: space-around; align-items: center">
            <div style="height: 300px">
                <el-steps direction="vertical" :active="activeindex">
                    <el-step :title="item" v-for="(item,index) in salaryItem" :key="index" />
...
<script>
    ...
    export default {
        ...
        methods: {
            init() {
                const _this = this;
                axios.get('http://10.16.104.183:8181/getSalaryAccount').then(function (resp) {
            ...
            deleteClick(row) {
                ...
                axios.post('http://10.16.104.183:8181/deleteSalaryAccount', JSON.parse(JSON.stringify(data))).then(function (resp) {
                    console.log(resp)
                    if (resp.data.code == "200") {//返回成功
                        _this.$message({
                            message: '删除成功',
            ...
            nextStep() {
                if (this.activeindex == 12) {
                    if (this.dialogVisible == true)//添加
                    {
                        ...
                        const _this = this
                        axios.post('http://10.16.104.183:8181/addSalaryAccount', this.salary).then(function (resp) {
                            // console.log(resp)
                            if (resp.data.code == "200") {//返回成功
                                _this.$message({
                                    message: '添加成功',
                    ...
                    if (this.editdialogVisible == true)//编辑
                    {
                        ...
                        const _this = this
                        axios.post('http://10.16.104.183:8181/editSalaryAccount', this.salary).then(function (resp) {
                            // console.log(resp)
                            if (resp.data.code == "200") {//返回成功
                                this.$message({
                                    message: '添加成功',
